...
def _call_netease_gateway(model, prompt, max_tokens=4096):
    """双通道 AI 调用：网易网关优先 → Gemini 直连自动降级"""
    # 通道1：网易 AI 网关（中国 IP 可用）
    try:
        resp = requests.post(
    ...
    # 通道2：Gemini 直连降级（全球可用）
    fallback_model = _GATEWAY_TO_GEMINI_FALLBACK.get(model, "gemini-2.5-pro")
    return _call_gemini_direct(fallback_model, prompt, max_tokens)
