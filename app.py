...
def render_browser_ai(model, prompt, max_tokens=4096, component_key="default"):
    """通过浏览器端 JS 调用网易 AI 网关。

    返回: str（成功） | "__ERROR__:msg"（失败） | None（等待中）
    """
    js_code = _build_fetch_js(model, prompt, max_tokens)
    raw = st_javascript(js_code, key=component_key)
    if raw is None or raw == 0:
        return None  # JS 还未执行完
    result = str(raw)
    if result.startswith("ERROR:"):
        return f"__ERROR__:{result[6:]}"
    return result
    ...
    if st.session_state.get("diag_running"):
        ...
        if text is not None:
            ...
            if not text.startswith("__ERROR__"):
                st.success(f"✅ 网关调用成功！返回: {text[:50]}")
            else:
                st.error(f"❌ 网关调用失败: {text}")
                st.warning(_BROWSER_FAIL_MSG)
            ...
            if trending:
                ...
                if st.session_state.get("trend_requesting"):
                    ...
                    if trend_text is not None:
                        if trend_text.startswith("__ERROR__"):
                            st.error(_BROWSER_FAIL_MSG)
    ...
    if mode == "💡 我有一个灵感/想法" and not lyrics_input:
        if can_generate_lyrics():
            ...
            if st.session_state.get("lyrics_requesting"):
                ...
                # 检查错误
                has_error = False
                if gemini_text and gemini_text.startswith("__ERROR__"):
                    has_error = True
                if opus_text and opus_text.startswith("__ERROR__"):
                    has_error = True

                if has_error:
                    st.error(_BROWSER_FAIL_MSG)
                    st.session_state["lyrics_requesting"] = False
                elif gemini_text is not None and opus_text is not None:
                ...
                if lyrics_input and not title:
                    ...
                    if st.session_state.get("name_requesting"):
                        ...
                        if name_raw is not None:
                            if name_raw.startswith("__ERROR__"):
                                st.warning("起名失败，请确保使用中国大陆网络。")
                            else:
                                st.session_state["auto_title"] = name_raw.strip()
